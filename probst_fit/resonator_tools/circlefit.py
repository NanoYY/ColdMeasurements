...
class circlefit(object):
    ...
    def _phase_fit(self,f_data,z_data,theta0, Ql, fr):
        ...
        fr = p_final[0]
        p0 = Ql
        p_final = spopt.leastsq(lambda a,b,c: residuals_4(a,b,c,theta0,fr), p0, args=(f_data,phase),ftol=1e-15,xtol=1e-15)
        Ql = p_final[0]
        p0 = [float(theta0), float(Ql), float(fr)]
        p0 = np.array(p0)
    ...
    def _fit_skewed_lorentzian(self,f_data,z_data):
        ...
        try:
            ...
            self.dQl_error = np.inf
        #return p_final[0]
        if (fra-popt[4])>0.01: print("WARNING: Perhaps the resonance frequency is not precisely determined", fra-popt[4])
        return popt
