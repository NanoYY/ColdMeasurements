...
class TemperatureController(GenericInstrument):
    ...
    def get_curve_data_point(self, curve, index):
        ...
        """
        curve_point = self.query(f"CRVPT? {curve},{index}").split(b",")
    ...
    def get_alarm_parameters(self, input_channel):
        ...
        """
        parameters = self.query(f"ALARM? {input_channel}").split(b",")
    ...
    def get_alarm_status(self, channel):
        ...
        separated_response = response.split(b",")
    ...
    def get_display_field_settings(self, field):
        ...
        """
        separated_settings = self.query(f"DISPFLD? {field}").split(b",")
    ...
    def _get_filter(self, input_channel):
        ...
        """
        filter_settings = self.query(f"FILTER? {input_channel}").split(b",")
    ...
    def get_keypad_lock(self):
        ...
        separated_response = output_string.split(b",")
    ...
    def get_min_max_data(self, input_channel):
        ...
        """
        min_max_data = self.query(f"MDAT? {input_channel}").split(b",")
    ...
    def _get_network_settings(self):
        ...
        """
        network_response = self.query("NET?").split(b",")
    ...
    def _get_network_configuration(self):
        ...
        """
        ethernet = self.query("NETID?").split(b",")
    ...
    def get_setpoint_ramp_parameter(self, output):
        ...
        """
        parameters = self.query(f"RAMP? {output}").split(b",")
    ...
    def get_relay_alarm_control_parameters(self, relay_number):
        ...
        """
        relay_config = self.query(f"RELAY? {relay_number}").split(b",")
    ...
    def get_relay_control_mode(self, relay_number):
        ...
        split_relay_settings = relay_settings.split(b",")
    ...
    def _get_tuning_control_status(self):
        ...
        """
        tuning_status = self.query("TUNEST?").split(b",")
    ...
    def _get_website_login(self):
        ...
        """
        login_response = self.query("WEBLOG?").split(b",")
        return {"username": login_response[0],
                "password": login_response[1]}

    def _get_identity(self):
        get_q = self.query('*IDN?', check_errors=False)
        return get_q.split(b',')
